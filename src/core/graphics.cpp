...
namespace January
{
    ...
    std::vector<uint32_t> VCompileGLSLToSPIRV(const std::string& sourceName, shaderc_shader_kind shaderKind, const std::string& glslSource, bool optimize)
    {
    ...
    VkPipelineLayout VCreatePipelineLayout(VkDevice device, VkDescriptorSetLayout descriptorSetLayout) 
    {
        ...
        // 2. Push Constants (For small, fast-updating pieces of data, e.g., a single matrix)
    ...
    VkPipeline VCreateGraphicsPipeline(VkDevice device, VkShaderModule vertShaderModule, VkShaderModule fragShaderModule,VkPipelineLayout pipelineLayout, VkRenderPass renderPass) 
    {
        ...
        // 2. Vertex Input State (Assuming hardcoded vertex data in shaders for simplicity)
        ...
        // This allows you to change resize handling without recreating the whole pipeline.
